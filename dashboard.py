...
def load_local_usage_stats():
    """Load usage stats from local files as backup"""
    usage_data = []
    
    # Check Twitter Slaves Natalie folder
...
def main():
    ...
    credentials_path = "Twitter Mass DM/google_service_account.json"
    drive_manager = None
    ...
    # Fallback to local files if Google Drive fails or no data
    if not all_usage_data:
        st.info("Loading from local files...")
        all_usage_data = load_local_usage_stats()
    
    if not all_usage_data:
        st.error("No usage data found. Please check your file paths and Google Drive access.")
        return
    ...
    # Auto-refresh toggle
    auto_refresh = st.sidebar.checkbox("🔄 Auto-refresh (30s)", help="Automatically refresh data every 30 seconds")
